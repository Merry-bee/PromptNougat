...
class PromptEncoder(nn.Module):
    ...
    def _embed_points(
        ...
        if pad: # padding_point:torch.tensor [bs,1,2]
            padding_point = torch.zeros((points.shape[0], 1, 2), device=points.device,dtype=torch.bfloat16)
    ...
    def forward(
        ...
        sparse_embeddings = torch.empty((bs, seq_len, 0, self.embed_dim), device=self._get_device(),dtype=torch.bfloat16) #[bs,len,0,d]
