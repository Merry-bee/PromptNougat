    ...
    for j,seg in enumerate(texts):
        ...
        while seg_idx < len(seg):            
            ...
            if tag_res:
                ...
                # 需要忽略后面括号内容的tag: \\label{sth}
                elif word in ['\\label','\\begin','\\end','\\includegraphics','\\resizebox','\\cline','\\multicolumn','\\multirow',
                              '\\input','\\bibliographystyle','\\bibliography','\\newcommand','\\usepackage','\\preprint','\\cite','\\ref','\\url','\\bibitem','\\bibinfo','\\bibnamefont']:
            ...
            invisible_res = re.match(r'(\{\[\})|_|\^|\||\{|\}|\$|\\\\|\\\s|\[.*\]|&|%|\s+|~|#|(natexlab)|(urlprefix)|\\(!|,|;)',seg[seg_idx:])
            if invisible_res:
                new_seg += invisible_res.group(0)  
                seg_idx += invisible_res.span()[1] 
                continue
            # visible符号：字母、数字、运算符、转义字符等: 加颜色
            visible_res = re.match(r'(\.*\s*[A-Za-z]+)|(\d+(,|\.)*)+|\+|-|/|>|<|=|\(|\)|\[|\]|,|\.|\*|@|;|!|\?|\:|\'|\"|`|(\\[^\\\s])',seg[seg_idx:])
            if visible_res:
                new_seg += colored_word(visible_res.group(0))
                seg_idx += visible_res.span()[1]
                continue
            else:
                raise ValueError('not handdled segment:',seg[seg_idx:])
...
def colored_file(tex_file):
    ...
    for i,line in enumerate(lines):
        if ('\\begin{document}') in line:
           ...
           break
    # if begin_idx == 0:
    #     raise Exception('No \\begin{document}')
...
def colored_dct(mmd_file):
    ...
    # 去掉多余空格和不统一的格式
    mmd = re.sub(r'\[RGB\]{(.*?)}',lambda x: x.group(0).replace(' ',''),mmd)
    mmd = re.sub(r'\[\s*RGB\s*]{\s*(\d{3})\s*,\s*(\d{3})\s*,\s*(\d{3})\s*}{\s*(\S+?)}',r'[RGB]\1,\2,\3\4',mmd)
...
def match_datapair(mmd_color_dct,pdf_file,png_dir,save_path):
    '''
    TODO:
    转为黑白图片
    '''
    ...
    for page_idx in range(len(pdf)):
        ...
        png_path = f'{png_dir}_{page_idx}.png'    # data/arxiv_train_data/png/0710.2897_0.png
        ...
        while len(fitz_color_dct) > 0:
            ...
            if mmd_color == (5,175,220):
                print(' '.join(pretext)[-100:]) 
...
def main(fold_lst):
    ...
    for i,tex_fold in enumerate(fold_lst[start_idx:]):
        ...
        # 分别对origin和color进行(后期可删）
        for dir in ['arxiv_origin','arxiv_color']:
            # 找到主文件
            tex_file = os.popen(f'find data/{dir}/{tex_fold} -maxdepth 1 -type f -name "*.tex"').read().split('\n')[0]
            # latexmk: tex->pdf（
            if os.system(f'latexmk -pdfps -f -interaction=nonstopmode -max-in-open=1000 -cd data/{dir}/{tex_fold} -output-directory=outputs {tex_file}'): 
                print(f'{file_idx}. latexmk error:{tex_fold}')
                
            # 去掉/usepackage{xcolor}
            if dir == 'arxiv_color':
                remove_package(tex_file)
            # tex->html
            if os.system(f'latexmlc --nocomments --includestyles --dest=data/{dir}/{tex_fold}/outputs/color.html --path=data/{dir}/{tex_fold} {str(Path(tex_file).name)}'):
                print(f'{file_idx}. latexmlc error:{tex_fold}')
                return
            # html->mmd
            if os.system(f'python nougat/dataset/parser/html2md.py --html data/{dir}/{tex_fold}/outputs/color.html --out data/{dir}/{tex_fold}/outputs/color.mmd'):
                print(f'{file_idx}. html2md error:{tex_fold}')
                return
        # mmd->dct_color
        ...
        # construct datapair
        match_datapair(dct_color,pdf_file,png_dir,save_path)
        
        # # 删除tex文件夹
        # if os.system(f'rm -r data/arxiv_origin/{tex_fold}'):
        #     print(f'{file_idx}. Remove error:{tex_fold}')
        #     return

    
if __name__ == '__main__':
    png_dir = 'data/arxiv_train_data/png/0710.2897'
    dct_color=colored_dct('data/arxiv_color/0710.2897/outputs/color.mmd')
    pdf_file='data/arxiv_color/0710.2897/outputs/susy07_proceeding_sqgl.pdf'
    save_path = 'data/arxiv_train_data/train.jsonl'
    match_datapair(dct_color,pdf_file,png_dir,save_path)
