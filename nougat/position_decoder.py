...
class PositionDecoder(nn.Module):
    ...
    def forward(self, heatmap,attention_valid_mask):
        ...
        # attention_valid_mask=0 -> pred=[[0,0],[0,0]]
        if attention_valid_mask.shape[1]==4095:   # train/validation with whole sentence
            attention_valid_mask = torch.cat((attention_valid_mask[:,1:],torch.zeros([bs,1]).to(x.device)),dim=1) # 需要向后移动一位
        valid_mask = attention_valid_mask.unsqueeze(-1).expand(-1,-1,4) #[bs,seq_len,4]
        x = torch.mul(x,valid_mask)   
        x1 = x[:,:,0]   # [bs,len]
        y1 = x[:,:,1]
        w = x[:,:,2]
        h = x[:,:,3]
...
def iou(pred,target,epsilon=1e-5):
    ...
    iou = (inter_area/(union_area+epsilon)).mean()
