...
class PromptBartForCausalLM(PromptBartPreTrainedModel):   
    ...
    def decode_position(self,cross_attn_weights,attention_mask):
        ...
        coords=self.position_decoder(cross_attn_weights,attention_mask) # [4,bs,16,len(input_ids),588]->[bs,len,2,2]
    ...
    def greedy_search(
        ...
        **model_kwargs, # attention_masks, encoder_outputs, use_cache
        ...
        if validation:
            ...
            next_token_logits = outputs.logits[:, -1, :] if prompt_in is None else outputs.logits  # logits: [bs,prompt_label_len,50000]
            ...
            while True:
                ...
                next_prompt = outputs.prompt_pred[:,-1:,:,:] #[bs,seq_len,2,2]
                prompt_in = torch.cat([prompt_in.to(next_prompt.device),next_prompt],dim=1).bfloat16()
...
class PromptDecoder(nn.Module):
    ...
    def prepare_inputs_for_inference(
        ...
        if past is not None:    # (n_layers,2+2,[bs,16,cur_len or 588,1024])
            ...
            # if past[0][0].shape[2] > 1:
            #     past = [list(past[l]) for l in range(len(past))]
            #     for l in range(len(past)):
            #         for i in range(2):
            #             past[l][i] = past[l][i][:,:,:-1,:]
            #     past = tuple([tuple(past[l]) for l in range(len(past))]) 
            # else: past = None
        output = {
            "input_ids": input_ids,
            "prompt_in": prompt_in,
