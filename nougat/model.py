...
class PromptBartDecoderLayer(nn.Module):
    ...
    def __init__(self, config: PromptBartConfig):
        ...
        '''
        self.prompt2image_attn = PromptAttention(
            embed_dim=self.prompt_embed_dim, 
            q_input_dim =self.prompt_embed_dim, 
            kv_input_dim=self.embed_dim,
            num_heads = config.decoder_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=True
        )
        self.prompt2image_attn_layer_norm = nn.LayerNorm(self.prompt_embed_dim)  # 256

        self.image2prompt_attn = PromptAttention(
            embed_dim=self.embed_dim,
        ...
        self.prompt2image_attn_layer_norm = nn.LayerNorm(self.embed_dim)  # 1024
        '''
    ...
    def forward(
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            attention_mask=attention_mask,              # [1,1,len(input_ids),len(input_ids)]: dim=-1,0:[0,0,...0] -> -1:[-inf,-inf,...0],0为mask,-inf为非mask
        ...
        '''
        # Prompt Cross-Attention Block： 修改image(encoder_hidden_states)

        if prompt_hidden_states is not None:    # 不输入prompt则跳过这个block
            # prompt2image:promt:Q;image:kv
            residual = prompt_hidden_states     # [bs,len,2,prompt_embed_dim]
            prompt_hidden_states = self.prompt2image_attn_layer_norm(prompt_hidden_states)
            
            bs,seq_len = prompt_hidden_states.shape[0],prompt_hidden_states.shape[1]
            out_prompts = torch.empty((bs, 0, 2, self.prompt_embed_dim), device=hidden_states.device,dtype=torch.bfloat16) #[bs,0,2,256]
            for idx in range(seq_len):
                prompt = prompt_hidden_states[:,idx,:,:]    # [bs,2,prompt_embed_dim]
                prompt, prompt2image_attn_weights, prompt2image_attn_present_key_value = self.prompt2image_attn(
                    hidden_states=prompt,                       # [bs,2,256]
                    key_value_states=encoder_hidden_states,     # [bs,588,1024]
                    past_key_value=None,    # 每次prompt都是新的，没有ache_kv
                    output_attentions=output_attentions,
                )
                
                out_prompts = torch.cat([out_prompts, prompt.unsqueeze(1)], dim=1)   # [bs,len,2,256]
            prompt_hidden_states = out_prompts
            prompt_hidden_states = nn.functional.dropout(prompt_hidden_states, p=self.dropout, training=self.training)
            prompt_hidden_states = residual+prompt_hidden_states    # [bs,len,2,prompt_embed_dim]
            
            # image2prompt:image:Q,prompt:kv
            '''
        ...
        if use_cache:
            if prompt_hidden_states is not None and cross_attn_past_key_value is not None: 
                present_key_value = present_key_value + cross_attn_past_key_value   # 有prompt，下一步仍然用之前的cross_attn_kv
            else:
        ...
        outputs = (hidden_states,)  # [bs,len(intput_ids),1024],

        if output_attentions:
            outputs += (self_attn_weights, cross_attn_weights)  # [bs,16,len(input_ids),cur_len], [bs,16,len(input_ids),588]

        if use_cache:
            outputs += (present_key_value,)         # ([bs,16,cur_len,64],[bs,16,cur_len,64],[bs,16,588,64],[bs,16,588,64])
...
class PromptBartDecoder(PromptBartPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if attention_mask is not None:  # 实现单向mask-self-attention
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
    ...
    def forward(
        ...
        if self.model.training:     # 使用cross_attention作为prompt box        
            ...
            if prompt is not None:
                if prompt.shape[1] != logits.shape[1]:  # 非全文prompt 
                    bs = logits.shape[0]    
            ...
            while not break_flag:
                ...
                logits = self.lm_head(outputs[0])   # [bs,1,50000]
                if validation: 
                    break
                if prompt_hidden_states is not None:
                    # plot attention
                    cross_attention = torch.stack(outputs.cross_attentions)[:,:,:,-1,:].permute(1, 0, 2, 3).squeeze(0).view(4, 16, 28, 21)
                    
                    frame = cross_attention.max(1)[0].max(0)[0].cpu().to(torch.float)
                    plt.clf()
                    plt.imshow(frame, cmap='viridis')
                    plt.savefig(f'output/tmp/{Path(pdf[0]).name}_{pdf[1]}_prompt.png')
                
                # 构造置信度低的数据并保存
                if pdf is not None: # inference
                    soft_logits = torch.softmax(logits,dim=2)
                    ...
                    for b in range(p_max.shape[0]):
                        ...
                        if p > p_thres:
                            break_flag=True
                            break
                        elif p_max.shape[0]==1: 
                            # batch_size=1,inference with human
                            print(f'pdf:{pdf[0]}\npretexts:{repr(self.tokenizer.decode(full_input_ids[b]))}\nnext_token:{self.tokenizer.decode(torch.argmax(logits[b], dim=-1)[-1])}\np:{p}')
                            # plot attention
                            cross_attention = torch.stack(outputs.cross_attentions)[:,:,:,-1,:].permute(1, 0, 2, 3).squeeze(0).view(4, 16, 28, 21)
                            frame = cross_attention.max(1)[0].max(0)[0].cpu().to(torch.float)
                            plt.clf()
                            plt.imshow(frame, cmap='viridis')
                            plt.savefig(f'output/tmp/{Path(pdf[0]).name}_{pdf[1]}.png')
                            prompt = input('box:')  # [[[x,y],[x,y]]]
                            if prompt=='y':
                                break_flag=True
                                break
                            prompt = eval(prompt)
                            prompt_hidden_states = self.prompt_encoder(points=None,boxes=torch.tensor(prompt,dtype=torch.bfloat16))
                            
                        else:
                            # batch_size>1,save data
                            filename = Path(pdf[0]).parents[1]/'json'/Path(pdf[0]).with_suffix('.json').name
                            page_idx = pdf[1]*p_max.shape[0]+b+1
                            pre_texts = self.tokenizer.decode(full_input_ids[b])
                            predicted_token = self.tokenizer.decode(torch.argmax(logits[b], dim=-1)[0])
                            with open(filename,"a") as fo:
                                json.dump({'page':page_idx,'pre_texts':pre_texts,'predicted_token':predicted_token,'p':p},fo)
                                fo.write('\n')

        cross_attn_weights = torch.stack(outputs['cross_attentions']) # [4, bs,16,len(input_ids),588]
        prompt_pred=torch.zeros_like(prompt[:,:-1,:,:])
        prompt_pred[:,:,0,0],prompt_pred[:,:,0,1],prompt_pred[:,:,1,0],prompt_pred[:,:,1,1]=self.position_decoder(cross_attn_weights) # [4,bs,16,len(input_ids),588]->[bs,len,2,2]
        
        loss,loss_token,loss_position = None,None,None
        ...
        return CausalLMOutput(
            loss=(loss,loss_token,loss_position),
            logits=logits,
    ...
    def greedy_search(
        ...
        **model_kwargs, # attention_masks, encoder_outputs, use_cache
        ...
        if validation:
            ...
            next_token_logits = outputs.logits[:, -1, :] if prompt is None else outputs.logits  # logits: [bs,prompt_label_len,50000]
            
            # pre-process distribution
            # next_token_scores = logits_processor(input_ids, next_token_logits)
            
            # next_token_logits:(batch_size, len, 50000); next_tokens: (batch_size, len)
            next_tokens = torch.argmax(next_token_logits, dim=-1)  
            # for b in range(next_tokens.shape[0]):
            #     start_idx = min(torch.where(input_ids[b]==1)[0])
            #     input_ids[b,start_idx:] = next_tokens[b,start_idx-1:-1]     # input_ids:[bs,len]
            ...
            while True:
                ...
                # ! 调用的是prepare_inputs_for_inference
                model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
                # model_inputs['input_ids']:[1,1], 只传入最后一个input_id
                # model_inputs['attention_mask']: 构造，None->[batch_size,cur_len]
                # model_inputs['past_key_values']:(n_layers,2+2,[bs,16,cur_len or 588,1024])
                # model_inputs['encoder_hidden_states']:[1,588,1024]
...
class PromptNougatModel(PreTrainedModel):
    ...
    def forward(
        ...
        # 有prompt时，只计算下一个token
        '''测试input和label对不对：
            tokenizer = PreTrainedTokenizerFast(tokenizer_file='checkpoints/0.1.0-small/tokenizer.json')
            input_ids = tokenizer('balabala')['input_ids']
            tokenizer.deocde(input_ids)
            '''
    ...
    def inference(
        ...
        '''
        generate函数内部还有一个判断repetition的地方 if stopping_criteria(input_ids, scores):->break
        我认为这里不需要，会引起empty误判，greedy_search()里注释掉了
        换decoding策略的时候也要注销
        '''
            ...
            if len(small_var) > 1:
                if np.all(np.diff(small_var) < 2):  # 判断small_var都是连续的；np.diff:后一个元素-前一个元素
                    ...
                    if idx / N > 0.85:  # at most last bit
                        output["repeats"].append(None)
                        continue
                    # elif small_var[0] < 30: # repetition出现在开头，认为整页缺失
                    #     idx = 0
                    logging.warn("Found repetitions in sample %i" % b)

                    # if pdf is not None:
                    #     filename = Path(pdf[0]).parents[1]/'json'/Path(pdf[0]).with_suffix('.json').name
                    #     page_idx = pdf[1]*batch_size+b+1
                    #     pre_texts = postprocess(self.decoder.tokenizer.decode(output["sequences"][b,:idx]),markdown_fix=False,)
                    #     pre_texts = re.sub(r'\[MISSING_.*?\]','',pre_texts)
                    #     predicted_token = self.decoder.tokenizer.decode(output["sequences"][b,idx])
                    #     with open(filename,"a") as fo:
                    #         json.dump({'page':page_idx, 'pre_texts':pre_texts,'predicted_token':predicted_token,'p':0},fo)
                    #         fo.write('\n')
